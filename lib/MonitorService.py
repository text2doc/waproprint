...
class MonitorService:
    ...
    def initialize(self):
        ...
        self.document_processor = DocumentProcessor(
            self.db_manager, printer_name, temp_folder)
    ...
    def start(self):
        ...
        if not self.db_manager.test_connection():
            logger.error(
                "Nie udało się połączyć z bazą danych. Usługa nie zostanie uruchomiona.")
            return False
    ...
    def log_startup_info(self):
        ...
        logger.info(
            f"Drukarka: {self.config_manager.get_thermal_printer_name()}")
        logger.info(
            f"Folder tymczasowy: {self.config_manager.get_temp_folder()}")

        if self.allowed_users:
            logger.info(
                f"Dozwoleni użytkownicy: {', '.join(self.allowed_users)}")
