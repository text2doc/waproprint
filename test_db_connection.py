...
def test_connection(config_path):
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "1. Verify the SQL Server is running and accessible from this machine")
            logger.error("2. Check if the server name/IP and port are correct")
            logger.error("3. Verify the database name is correct")
            logger.error(
                "4. Check if the SQL Server allows remote connections")
            logger.error("5. Verify the username and password are correct")
            logger.error(
                "6. Check if the SQL Server port (default 1433) is not blocked by firewall")
            logger.error(
                "7. Try running: telnet <server_ip> 1433 to test basic connectivity")
            return False
